...
func (uidSet *UIDSet) ToUids() []uint64 {
	...
	//println("uidSet size:", uidSet.NumUids(), "Len result:", len(result), "UIDs:", result)
	return result
}
...
func (uidSet *UIDSet) NewIterator() *UidSetIterator {
	...
	if bitmap, ok := (*uidSetIt.bitmaps)[base]; ok {
		...
		uidSetIt.itr = bitmap.ManyIterator()
	}
	return uidSetIt
}

func (uidSetIt *UidSetIterator) Next(uids []uint64) int {
	if uidSetIt == nil {
		return 0
	}
	if uidSetIt.curIdx >= len(uidSetIt.bases) {
		return 0
	}

	// Adjust size of uidSetIt.many.
	if len(uids) > cap(uidSetIt.many) {
		uidSetIt.many = make([]uint32, 0, len(uids))
	}
	uidSetIt.many = uidSetIt.many[:len(uids)]

	fill := func(base uint64) int {
		if uidSetIt.itr == nil {
			return 0
		}
		out := uidSetIt.itr.NextMany(uidSetIt.many)
		for i := 0; i < out; i++ {
			...
			uids[i] = base | uint64(uidSetIt.many[i])
		}
		return out
	}

	for ; uidSetIt.curIdx < len(uidSetIt.bases); uidSetIt.curIdx++ {
		base := uidSetIt.bases[uidSetIt.curIdx]
		if sz := fill(base); sz > 0 {
			return sz
		}
		uidSetIt.itr = nil
		base = uidSetIt.bases[uidSetIt.curIdx]
